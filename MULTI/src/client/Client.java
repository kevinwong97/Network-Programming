...
public class Client {
	...
	public static void main(String[] args) {
		...
		try {
			...
			socket = new Socket();
			socket.connect(new InetSocketAddress(serverAddress, portNum), TIME_OUT);
			socket.setSoTimeout(TIME_OUT);
			System.out.println("Connected to " + serverAddress + " on port Number " + portNum);
			...
			do {
				...
				// print msg and read from console from Response object
				if (command.getType() == Response.READ_LINE) {
		...
		// class not found only because the project is not compiled wholey. 
		catch (ClassNotFoundException e) {
			System.err.println("The command sent from server cant be read by this client. " + e.getMessage());
		}
