...
public class ServerCallbackImpl implements ServerCallback {
	 ...
	 */
	@Override
	public void scbServerInitiated(SinglePlayerServer server, int portNum) {
		SERVER_INITIATED("Server initiated on port Number " + portNum);
	}
	private void SERVER_INITIATED(String message) {
		System.out.println("SERVER INITIATED - " + message);
	 ...
	 */
	@Override
	public void scbClientConnected(SinglePlayerServer server, Socket socket) {
	 ...
	 */
	@Override
	public void scbClientDisconnected(SinglePlayerServer server, Socket socket, ServerProcess process) {
